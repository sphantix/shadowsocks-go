...
func handShake(conn net.Conn) (err error) {
	...
	if buf[idVer] != socksVer5 {
		log.Println("socks version is not match %v buf %v", int(buf[idVer]), buf)
		return errVer
	}
	...
	if n == msgLen { // handshake done, common case
		...
		log.Println("n == msgLen handle shake finished ", n)
	} else if n < msgLen { // has more methods to read, rare case
		if _, err = io.ReadFull(conn, buf[n:msgLen]); err != nil {
			return
		}
		log.Println("handle shake read not finished n ", n, " len", msgLen)
	} else { // error, should not get extra data
		return errAuthExtraData
	}
	// send confirmation: version 5, no authentication required
	log.Println("write handle shake to local browser, handle shake finished ")
...
func getRequest(conn net.Conn) (rawaddr []byte, host string, err error) {
	...
	if buf[idVer] != socksVer5 {
		log.Println("socks version : ", int(buf[idVer]))
		err = errVer
		return
	}
	if buf[idCmd] != socksCmdConnect {
		err = errCmd
		log.Println("socks command is not right ", int(buf[idCmd]))
		return
	}
...
func handleConnection(conn net.Conn) {
	...
	//reply to client the result: succeed and local server ip:0.0.0.0 port 1080
	...
	log.Println("create connection to server . target addr ", string(rawaddr))
