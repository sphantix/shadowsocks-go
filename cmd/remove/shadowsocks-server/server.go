...
func getRequest(conn *ss.Conn, auth bool) (host string, ota bool, addrType byte, err error) {
	...
	log.Println("start to read request iv")
	...
	if auth || addrType&ss.OneTimeAuthMask > 0 {
		...
		iv := conn.GetIv()
		key := conn.GetKey()
		//log.Println("get iv %v key %v", iv, key)
...
func handleConnection(conn *ss.Conn, auth bool, port string) {
	...
	host, ota, addrType, err := getRequest(conn, auth)
	if err != nil {
	...
	//这里可以加入连接本地的代理转发服务器
	proxyServerHostPort := net.JoinHostPort(proxyServer, strconv.Itoa(proxyServerPort))

	proxyServerConn, err := net.Dial("tcp", proxyServerHostPort)
	if err != nil {
		log.Printf("connect to proxy server %v failed \n", proxyServerHostPort)
		return
	}

	//发送目的地给代理转发服务器
	targetHostLen := len(host)
	buf := make([]byte, targetHostLen+2)
	buf[idType] = addrType //设置地址类型
	buf[idDmLen] = byte(targetHostLen)
	copy(buf[idDmLen+1:], []byte(host))
	_, err2 := proxyServerConn.Write(buf)
	if err2 != nil {
		log.Printf("write host to target proxy server failed \n")
		return
	}
	...
	if debug {
		debug.Printf("piping %s<->%s ota=%v connOta=%v target addr %s ", proxyServerHostPort,
			sanitizeAddr(conn.RemoteAddr()), ota, conn.IsOta(), host)
	}
	if ota {
		go func() {
			ss.PipeThenCloseOta(conn, proxyServerConn, func(flow int) {
				passwdManager.addFlow(port, flow)
			})
		}()
	} else {
		go func() {
			ss.PipeThenClose2(conn, proxyServerConn, func(flow int) {
				passwdManager.addFlow(port, flow)
			}, host)
		}()
	}
	ss.PipeThenClose2(proxyServerConn, conn, func(flow int) {
		passwdManager.addFlow(port, flow)
	}, host)
...
func main() {
	...
	flag.StringVar(&cmdConfig.ProxyServer, "proxy", "127.0.0.1", "proxy server ip address")
	flag.IntVar(&cmdConfig.ProxyServerPort, "proxy_port", 8877, "proxy server port")
	flag.Parse()
	...
	proxyServer = cmdConfig.ProxyServer
	proxyServerPort = cmdConfig.ProxyServerPort
